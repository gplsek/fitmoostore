/*
* joebark Scripts
* Author: brian.anarell@gmail.com
*/
...
/* usage if(jQuery.browser.mobile) */

/* toggle input vals */
$.fn.ToggleInputValue = function(){return $(this).each(function(){var default_value, Input = $(this);default_value = Input.val();Input.focus(function() {if(Input.val() == default_value) {Input.val("");}}).blur(function(){if(Input.val().length == 0) {Input.val(default_value);}});});};

/* number format */
function numberWithCommas(x) {return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");}

/* naturalwidth / height jquery plugin */
...
var fitmoo = {
  ...
  customSelects:function() {
    var _this, sels, thisParent;
    $('form').each(function() {
      thisForm = $('#' + $(this).attr('id'));
      sels = thisForm.find('select');
      if(sels.length) {
      	sels.each(function() {
      		_this = $(this);
      		thisParent = _this.closest('.form-item');
        	_this.sb({
          	fixedWidth: false,
          	ddCtx: thisParent
        	});
